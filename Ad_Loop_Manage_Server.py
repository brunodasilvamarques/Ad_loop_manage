...
def heartbeat():
    ...
    try:
        master_summary_cache[anydesk_id] = summarize_videos_from_master(anydesk_id)
    except Exception:
        master_summary_cache[anydesk_id] = {}
...
def dashboard():
    ...
    for anydesk_id in pick_unique_kiosk_ids():
        ...
        info["last_heartbeat_iso"] = info.get("last_seen")
        ...
        for v in heartbeat_videos:
            ...
            heartbeat_by_name[fn] = {
                "play_count": int(v.get("play_count", 0) or 0),
                "total_play_duration": int(v.get("total_play_duration", 0) or 0),
                "first_play": v.get("first_play"),
                "last_play": v.get("last_play"),
                "active": bool(v.get("active", True))
            }

        all_names = sorted(set(ms.keys()) | set(heartbeat_by_name.keys()), key=lambda x: x.lower())

        video_list = []
        for fn in all_names:
            rec = ms.get(fn, {})
            hb = heartbeat_by_name.get(fn, {})
            has_hb = fn in heartbeat_by_name

            rec_play_count = int(rec.get("play_count", 0) or 0)
            rec_total_duration = int(rec.get("total_duration", 0) or 0)

            hb_play_count = int(hb.get("play_count", 0) or 0)
            hb_total_duration = int(hb.get("total_play_duration", 0) or 0)

            play_count = hb_play_count if has_hb else rec_play_count
            total_play_duration = hb_total_duration if has_hb else rec_total_duration

            first_play_raw = hb.get("first_play") if has_hb and hb.get("first_play") else rec.get("first_play")
            last_play_raw = hb.get("last_play") if has_hb and hb.get("last_play") else rec.get("last_play")

            item = {
                "filename": fn,
                "play_count": play_count,
                "total_play_duration": total_play_duration,
                "first_play": (first_play_raw.replace(" ", "T") if first_play_raw else None),
                "last_play": (last_play_raw.replace(" ", "T") if last_play_raw else None),
                "active": bool(hb.get("active", False)) if has_hb else False,
            }
