...
def heartbeat():
    ...
    if kiosk_name or kiosk_code or country:
        ...
        changed = (not existing) or (prev_name != new_name) or (prev_ctry != new_country) or (prev_code != new_code)
        if changed:
            kiosk_mappings[anydesk_id] = {"kiosk_name": new_name, "country": new_country, "kiosk_code": new_code}
            save_mappings()

            # If this was a real change (not first-time empty → value), notify & suppress offline for a bit
            if existing and (prev_name != new_name or prev_ctry != new_country or prev_code != new_code):
                kiosks.setdefault(anydesk_id, {})
                kiosks[anydesk_id]["identity_changed_at"] = now.isoformat()
                # suppress offline alerts for 10 minutes
                kiosks[anydesk_id]["suppress_offline_until"] = (now + timedelta(minutes=10)).isoformat()
                save_kiosks()

                subject = f"🆕 VLS Identity Updated | {new_country} - {new_name}"
                body = (
                    "The kiosk identity was updated.\n\n"
                    f"Kiosk ID: {anydesk_id}\n"
                    f"Old Name: {prev_name or 'None'} → New Name: {new_name}\n"
                    f"Old Country: {prev_ctry or 'None'} → New Country: {new_country}\n"
...
def dashboard():
    ...
    for anydesk_id, info in kiosks.items():
        # Attach weekly uptime from master_data
        try:
        ...
        # Prefer hourly cache; fall back to on-demand scan if cache empty/missing
        ms = master_summary_cache.get(anydesk_id)
        if not ms:  # None or empty dict
            ms = summarize_videos_from_master(anydesk_id)

        # Always use master_data-derived list; never use the heartbeat copy
        info["videos"] = [
            {
                "filename": fn,
                "play_count": rec["play_count"],
                "total_play_duration": rec["total_duration"],
                # Normalize to ISO (so client JS can parse), even if source has a space
        ...
        # NEW: pretty country label (GBP→United Kingdom etc.)
        iso_to_name = {"GBP": "United Kingdom", "USD": "United States of America", "EUR": "Europe"}
        if mapping:
...
def download_csv():
    ...
    for anydesk_id, info in kiosks.items():
        mapping = kiosk_mappings.get(anydesk_id, {"kiosk_name": anydesk_id, "country": "Unknown"})
        # Build from uploaded master files only
        summary = master_summary_cache.get(anydesk_id)
        if not summary:
            summary = summarize_videos_from_master(anydesk_id)

        for filename, rec in (summary or {}).items():
            writer.writerow([
                kiosk_mappings.get(anydesk_id, {}).get('country', 'Unknown'),
                kiosk_mappings.get(anydesk_id, {}).get('kiosk_name', anydesk_id),
                filename,
    ...
    try:
        ...
        threading.Thread(
            ...
            args=("📊 Weekly ChangeBox Advertisement Video Playback CSV Report", csv_bytes, [RECIPIENT_EMAIL]),
            daemon=True,
        ).start()
    except Exception as e:
    ...
    output.seek(0)
    return Response(
        output.getvalue(),  # send text, not the StringIO object
        mimetype="text/csv",
...
def download_uptime_csv():
    ...
    output = StringIO()
    writer = csv.writer(output)
    # Country, Code, Name, Date, UpHours, 00:00..23:00
    headers = ["Country", "Kiosk Code", "Kiosk Name", "Date", "Up Hours"] + [f"{h:02d}:00" for h in range(24)]
    writer.writerow(headers)

    for anydesk_id, info in kiosks.items():
        mapping = kiosk_mappings.get(anydesk_id, {"kiosk_name": anydesk_id, "country": "Unknown", "kiosk_code": None})
        weekly = compute_weekly_uptime_from_master(anydesk_id)["days"]
        for date_key, hours_map in weekly.items():
            vals = [int(hours_map.get(f"{h:02d}:00", 0)) for h in range(24)]
            writer.writerow([
                mapping["country"], mapping.get("kiosk_code") or "N/A", mapping["kiosk_name"],
                date_key, sum(vals), *vals
            ])
    ...
    try:
        ...
        try:
            ...
            if device_id:
                ...
                changed = (not existing) or (prev_name != new_name) or (prev_ctry != new_country) or (prev_code != new_code)
                if changed:
                    kiosk_mappings[device_id] = {"kiosk_name": new_name, "country": new_country, "kiosk_code": new_code}
                    save_mappings()
...
def send_weekly_csv_email():
    ...
    email_data = {
        "message": {
            ...
            "body": {
                "contentType": "Text",
                "content": "Attached is the combined CSV from all kiosks"
            },
            "toRecipients": [
                {
                    "emailAddress": {
