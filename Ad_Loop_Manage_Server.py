...
def heartbeat():
    ...
    if pi_id not in kiosks:
        ...
        # Update uptime based on elapsed time since last heartbeat
        last_seen_str = kiosks[pi_id].get("last_seen")
        if last_seen_str:
            last_seen = datetime.fromisoformat(last_seen_str)
            delta = datetime.fromisoformat(now) - last_seen
            kiosks[pi_id]["uptime_seconds"] = kiosks[pi_id].get("uptime_seconds", 0) + delta.total_seconds()

        kiosks[pi_id]["last_seen"] = now
        kiosks[pi_id]["software_version"] = software_version
        kiosks[pi_id]["videos"] = videos
