...
UPTIME_SAMPLE_INTERVAL_SECONDS = 3600      # record one uptime point per hour
UPTIME_MATCH_TOLERANCE_SECONDS = 600       # allow 10 minutes tolerance around expected heartbeat
...
def heartbeat():
    ...
    # Determine if it's time to record an uptime point (every 1 hour)
    new_uptime_entry_added = False
    history = kiosk["uptime_history"]
    interval = UPTIME_SAMPLE_INTERVAL_SECONDS
    ...
    if (now - last_record_time).total_seconds() >= interval:
        # Match if any actual heartbeat is within the allowed tolerance window
        matched = any(
            abs((now - datetime.fromisoformat(at)).total_seconds()) <= UPTIME_MATCH_TOLERANCE_SECONDS
...
def compute_weekly_uptime_from_master(anydesk_id: str):
    ...
    # 1) Merge uptime_hours from all supported data files
    by_date = {}
    for path in files:
        try:
            with open(path, "r", encoding="utf-8") as f:
            ...
            for date_key, day in _iter_daily_sections_from_payload(j):
                by_date.setdefault(date_key, {})
                uh = day.get("uptime_hours", {})
                if isinstance(uh, dict):
                    by_date[date_key].update({k: 1 if int(v) == 1 else 0 for k, v in uh.items()})
...
def compute_all_uptime_from_master(anydesk_id: str) -> dict[str, dict[str, int]]:
    """
    Return every day we can find in uploaded data files for this kiosk:
    ...
    for path in files:
        try:
            ...
            for date_key, day in _iter_daily_sections_from_payload(j):
                hours = day.get("uptime_hours", {})
                if isinstance(hours, dict):
                    d = all_days.setdefault(date_key, {})
...
def update_all_kiosk_uptime():
    ...
    for kiosk in kiosks.values():
        ...
        if (now - last_record_time).total_seconds() >= interval:
            # Check if any heartbeat matches within the allowed tolerance window
            matched = any(
                abs((now - datetime.fromisoformat(at)).total_seconds()) <= UPTIME_MATCH_TOLERANCE_SECONDS
                for at in kiosk["actual_heartbeat_times"]
            )
