...
def heartbeat():
    ...
    if kiosk_name or kiosk_code or country:
        ...
        new_name    = kiosk_name or existing.get('kiosk_name') or anydesk_id
        new_code    = kiosk_code or existing.get('kiosk_code')
        if (not existing) or (existing.get('kiosk_name') != new_name) or (existing.get('country') != new_country) or (existing.get('kiosk_code') != new_code):
...
def upload_json():
    ...
    try:
        ...
        try:
            ...
            if device_id:
                # Auto-map name/country if present
                existing   = kiosk_mappings.get(device_id, {})
                new_name   = kname or existing.get("kiosk_name") or device_id
                new_country= kcountry or existing.get("country") or "Unknown"
                new_code   = kcode or existing.get("kiosk_code")
                if (not existing) or (existing.get("kiosk_name") != new_name) or (existing.get("country") != new_country) or (existing.get("kiosk_code") != new_code):
