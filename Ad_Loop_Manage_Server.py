...
def heartbeat():
    ...
    if anydesk_id not in kiosks:
        ...
        raw_first_seen = kiosks[anydesk_id].get("first_seen", now.isoformat())
        first_seen = datetime.fromisoformat(raw_first_seen) if isinstance(raw_first_seen, str) else raw_first_seen
        interval = 60  # 1 min
        expected_times = []
        t = first_seen
        while t < now:
        ...
        raw_uptime_pct = (matches / expected_count) * 100 if expected_count else 0
        kiosks[anydesk_id]['uptime_percent'] = round(min(raw_uptime_pct, 100), 2)
