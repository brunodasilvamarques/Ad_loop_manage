...
# ---- NEW: normalization helpers (non-breaking) --------------------------------
COUNTRY_ALIASES = {
    # UK
...
def _canon_country(v):
    if v is None:
        return None
    key = _norm_text(v).casefold()
    # prefer ISO code we understand
    return COUNTRY_ALIASES.get(key, _norm_text(v).upper())
    ...
    if kiosk_name or kiosk_code or country or kiosk_address:
        ...
        if changed:
            ...
            if existing and prev_fp != new_fp:
                ...
                # NEW: dedupe using last stored fingerprint
