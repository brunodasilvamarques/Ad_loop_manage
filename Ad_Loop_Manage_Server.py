...
def dashboard():
    ...
    for anydesk_id in pick_unique_kiosk_ids():
        ...
        for fn in all_names:
            ...
            first_candidates = [x for x in [rec.get("first_play"), hb.get("first_play")] if x]
            last_candidates = [x for x in [rec.get("last_play"), hb.get("last_play")] if x]

            first_play = min(first_candidates) if first_candidates else None
            last_play = max(last_candidates) if last_candidates else None

            item = {
                "filename": fn,
                "play_count": rec_play_count + hb_play_count,
                "total_play_duration": rec_total_duration + hb_total_duration,
                "first_play": (first_play.replace(" ", "T") if first_play else None),
                "last_play": (last_play.replace(" ", "T") if last_play else None),
                "active": bool(hb.get("active", False)),
            }
