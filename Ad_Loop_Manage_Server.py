...
def heartbeat():
    ...
    if anydesk_id not in kiosks:
        ...
        kiosks[anydesk_id].setdefault("heartbeats", []).append(now.isoformat())

        # Keep track of all expected timestamps within 7-day window
        first_seen = datetime.fromisoformat(kiosks[anydesk_id].get("first_seen", now.isoformat()))
        interval = 60  # 1 min
        expected_times = []
        t = first_seen
        while t < now:
        ...
        recent_expected = [t for t in expected_times if t > seven_days_ago]
        recent_actual = [
            datetime.fromisoformat(hb)
            for hb in kiosks[anydesk_id]["heartbeats"]
            if datetime.fromisoformat(hb) > seven_days_ago
        ]

        # Count actual matches: ±90 sec tolerance
        matches = 0
        for et in recent_expected:
            if any(abs((et - at).total_seconds()) <= 90 for at in recent_actual):
                matches += 1

        expected_count = len(recent_expected)
        kiosks[anydesk_id]['uptime_percent'] = round((matches / expected_count) * 100, 2) if expected_count else 0
...
def dashboard():
    ...
    for anydesk_id, info in kiosks.items():
        ...
        actual = len(heartbeats)

        # Determine how long the kiosk has been known
        ...
        duration_seconds = (datetime.utcnow() - first_seen).total_seconds()
        expected = max(1, duration_seconds / 60)  # 1 ping per minute

        # Uptime = received / expected
        info['uptime_percent'] = round((actual / expected) * 100, 2)
