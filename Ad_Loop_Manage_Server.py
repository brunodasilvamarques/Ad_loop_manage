...
def dashboard():
    ...
    for anydesk_id, info in kiosks.items():
        ...
        # Prefer hourly cache; fall back to on-demand scan if missing
        ms = master_summary_cache.get(anydesk_id) or summarize_videos_from_master(anydesk_id)
        if ms:
            info["videos"] = [
                {
                    "filename": fn,
                    ...
                    "first_play": rec["first_play"],
                    "last_play": rec["last_play"],
                }
...
def download_csv():
    ...
    for anydesk_id, info in kiosks.items():
        mapping = kiosk_mappings.get(anydesk_id, {"kiosk_name": anydesk_id, "country": "Unknown"})
        # Prefer hourly cache; fall back to on-demand scan if needed
        summary = master_summary_cache.get(anydesk_id) or summarize_videos_from_master(anydesk_id)

        if summary:
            for filename, rec in summary.items():
                writer.writerow([
                    mapping['country'],
                    mapping['kiosk_name'],
                    filename,
            ...
            # fallback — shouldn’t be needed once master_data exists
